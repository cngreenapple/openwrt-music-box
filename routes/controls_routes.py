...
from app import mpv_send, owrtmb_state, state_lock, af_state, update_mpv_filters, EQ_PRESETS, generate_fireq_cmd, trigger_play, play_next_in_queue
...
def toggle_bitperfect():
    ...
    if os.path.exists(BP_MODE_FILE):
        try: current = open(BP_MODE_FILE).read().strip()
        except: pass
    new_state = "1" if current == "0" else "0"
    open(BP_MODE_FILE, 'w').write(new_state)
    update_mpv_filters()
    if new_state == "0":
...
def get_bitperfect():
    ...
    active = False
    if os.path.exists(BP_MODE_FILE):
        active = open(BP_MODE_FILE).read().strip() == "1"
