// OwrtBox Player - CORE v5 (All Features)
...
let lyricsData = [], lyricsType = '', lastLyricsTitle = '', isMuted = false, prevVolume = 50;
let audioContext = null, analyserNode = null, sourceNode = null, eqCanvas = null, eqCtx = null;
let eqAnimId = null;
...
window.onload = () => {
    ...
    eqCanvas = document.getElementById('eq-visualizer');
    ...
    if(browserAudio) {
        ...
        browserAudio.addEventListener('play', () => { if(!audioContext) initAudioVisualizer(); });
    }
    
    updateUI(); setupKnobs(); checkBitPerfect(); checkCrossfeed(); initPath();
    initVol(); initDragDrop(); initKeyboard();
    ...
    if(vs) { vs.value = settings.vol||50; vs.addEventListener('input', () => { const v=parseInt(vs.value); settings.vol=v; localStorage.setItem('owrtmb_set',JSON.stringify(settings)); document.getElementById('vol-val').textContent=v; if(!isMuted) { if(systemState.playMode==='browser'&&browserAudio) browserAudio.volume=v/100; else fetch('/control/volume?val='+v); } }); }
};
...
function initKeyboard() {
    document.addEventListener('keydown', (e) => {
        ...
        switch(e.code) {
            ...
            case 'ArrowUp': showToast('Volume Up'); fetch('/control/volume?val='+Math.min(100,(settings.vol||0)+5)); break;
            case 'ArrowDown': showToast('Volume Down'); fetch('/control/volume?val='+Math.max(0,(settings.vol||0)-5)); break;
...
function toggleMute() {
    ...
    updateMuteBtn();
    if(isMuted) {
        fetch('/control/volume?val=0');
        if(browserAudio) browserAudio.volume = 0;
    } else {
        fetch('/control/volume?val='+(settings.vol||50));
        if(browserAudio) browserAudio.volume = (settings.vol||50)/100;
    }
}
...
function initAudioVisualizer() {
    try {
        ...
        sourceNode = audioContext.createMediaElementSource(browserAudio);
...
function drawEQ() {
    if(!analyserNode || !eqCtx || !eqCanvas) return;
    ...
    const draw = () => {
        ...
        const w = eqCanvas.width / bufferLength * 2;
        let x = 0;
        for(let i=0; i<bufferLength; i++) {
            const h = (dataArray[i] / 255) * eqCanvas.height;
            eqCtx.fillStyle = '#1db954';
            eqCtx.fillRect(x, eqCanvas.height - h, w-1, h);
            x += w;
        }
    };
    draw();
...
function initDragDrop() {
    ...
    if(!zone) return;
    zone.addEventListener('dragover', (e) => { e.preventDefault(); if(overlay) overlay.style.display='flex'; });
    zone.addEventListener('dragleave', () => { if(overlay) overlay.style.display='none'; });
    zone.addEventListener('drop', (e) => {
        ...
        if(files.length) {
            ...
            if(input) { input.files = files; uploadFiles({target:{files:files}}); }
        }
    });
}
...
function playRadio(url) {
    ...
    if(browserAudio) {
        ...
        setText('tit', 'Radio Stream');
        setText('art', url.substring(0,40)+'...');
...
function onDrag(e) {
    ...
    if(type==='vol') {
        let val=0; if(deg>=210) val=((deg-210)/300)*100; else if(deg<=150) val=((150+deg)/300)*100; else val=(Math.abs(deg-210)<Math.abs(deg-150))?0:100;
...
function updateUI() {
    ...
    const vk=document.getElementById('k-vol'); if(vk){const deg=((settings.vol/100)*270)-135;vk.style.transform='rotate('+deg+'deg)';}
    document.getElementById('v-vol-val').textContent=settings.vol+'%'; localStorage.setItem('owrtmb_set',JSON.stringify(settings));
}
        ...
        if(mode==='play_now') {
            ...
            if(systemState.playMode==='browser') {
                if(url.includes('youtube')||url.includes('youtu.be')){
                    fetch('/youtube_audio?url='+encodeURIComponent(url)).then(r=>r.json()).then(yt=>{if(yt.audio_url){setText('tit',yt.title||title);if(yt.thumbnail)document.getElementById('cover-img').src=yt.thumbnail;if(browserAudio){browserAudio.src='/youtube_proxy?url='+encodeURIComponent(url);browserAudio.volume=(settings.vol||50)/100;browserAudio.play().catch(()=>{});isPlaying=true;updatePlayBtn();}}}).catch(()=>{});
...
function pollStatus() {
    fetch('/status').then(r=>r.json()).then(d=>{
        if(systemState.playMode!=='browser'){
            ...
            ['genre','year'].forEach(id=>{const el=document.getElementById(id);if(el){el.textContent=d[id]||'';el.style.display=d[id]?'inline-flex':'none';}});
